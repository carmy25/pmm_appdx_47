...
def format_header(ws):
    ...
    header_cell_center_border(ws, "g1", 'Рахується')
    header_cell_center_border(ws, "h1", 'Проведено ФЕС')
    header_cell_center_border(ws, "i1", '% ФЕС')

    ws.merged_cells.ranges.add("j1:k1")
    header_cell_center_border(ws, "j1", 'Підсумок')


def format_summary(ws, total_by_category):
    cell_center_border(ws, 'J2', 'ДП')
    cell_center_border(ws, 'J3', 'АБ')
    cell_center_border(ws, 'j4', 'МіМ')
    cell_center_border(ws, 'j5', 'Керосин')

    cell_center_border(ws, 'k2', total_by_category['DIESEL'])
    cell_center_border(ws, 'k3', total_by_category['PETROL'])
    cell_center_border(ws, 'k4', total_by_category['OIL'] + total_by_category['POISON'])
    cell_center_border(ws, 'k5', total_by_category['KEROSENE'])
...
def export_fals_report(ws, fal_types):
    ...
    for i, fal_type in enumerate(fal_types, 2):
        ...
        cn = f'g{i}'
        cell_center_border(ws, cn, f'=b{i}+c{i}-d{i}-e{i}-f{i}')

        cell_center_border(ws, f'I{i}', f'=h{i}/g{i} * 100')
